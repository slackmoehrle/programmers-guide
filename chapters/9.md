...
>>>>>>> 5ebafbabb85aeb65be0274fb6113cbd022630a3f
...
In the most cases you will have multiple textures per model, unwrapped in a texture
atlas.
...
* _Texture_ - All surfaces and vertices of a 3D model can be mapped to a texture. In most cases you will have multiple textures per model, unwrapped in a texture atlas.

* _Camera_ - Since a 3D world is not flat, you need to set a camera to look at it. You get different scenes with different camera parameters.
...
<<<<<<< HEAD
You can attach 3D models to other 3D models to create rich effects. An example
would be adding a weapon to a character. To do this you need to find the attachment
point where the weapon is to be added. For this use the
`getAttachNode(attachment_point_name)` function. And then we jus add the new model
to the attachment point as a child with `addChild()`. You can think of this as
...
<<<<<<< HEAD
methods in the model production, our built in tool, `fbx-conv` will generate additional
keyframes to compensate. This compensation is completed in accordance with the
target frame. For more information about `fbx-conv` please refer to the section
discussing it at the end of this chapter.
=======
...
<<<<<<< HEAD
## Terrain
`Terrain` is an important component in a 3D game. A __texture__ is used to act
for the height map. Up to 4 textures can be used to blend the details of the
terrain, grass, road, and so on.

### HeightMap
__HeightMap__ is the core of a `Terrain` object. Different from the common image,
the height map  represents the height of vertices. It determines terrain's
geometry shape.

### DetailMap
`DetailMap` is a list of textures that determine the details of the terrain, up
to four textures can be used.
=======
##Terrain
...
###HeightMap
heightmap is the core of the terrain. Different from the common image the height map represents the height of vertices. It determines the terrain's geometry shape.

###DetailMap
DetailMaps are a list of textures determining the details of the terrain, up to four textures can be used.

###AlphaMap
AlphaMap is an image whose data is the blend weights of detailMaps. The blending result is the final terrain's appearance.
>>>>>>> 5ebafbabb85aeb65be0274fb6113cbd022630a3f
...
* pass the `TerrainData` object to `Terrain::create`, the last parameter determines
...
<<<<<<< HEAD
* set the `Terrain` objects __camera mask__ and add it to a `Node` or a `Scene`.
Be careful, when `Terrain` is added into a `Node` or a `Scene`, you can not
perform a `transform(translate, scale)` on it any more. If you do that after
`addChild()`, the `Terrain` object's methods may calculate wrong result.
=======
* set terrain camera mask, add it to a node or a scene. Be careful, when terrain is added into a node or a scene, you can not transform(translate, scale) it any more. If you do that after `addChild()`, some of the terrain's methods may calculate wrong results.

###LOD policy
Terrain use `the level-of-detail(LOD)`, to reduce further parts' vertices. Users can set the distance to camera which occur the terrain's chunks LOD changing is done through the:
...
<<<<<<< HEAD
`Billboard` objects are easy to create. `BillBoard` is derived from `Sprite`,
so it supports most of the features as a `Sprite` object. We can create for using
the following create methods:
=======
...
<<<<<<< HEAD
You can also create a `Billboard` for the camera XOY plane (thinking like
  the plane of a floor or ground surface) by changing the `BillBoard` objects mode:
=======
