...
market and Cocos2d-x is adding all the features you need for 3D development. 3D development might be new to you and use some terminology that you are unfamiliar with. There are also additional softare tools that you need to become familiar with. Let's jump right in and get our feet wet.
...
## Sprite3D
Just like 2D games, 3D games also have `Sprite` objects. `Sprite` objects are a core foundation of any game.
`Sprite3D` has 3 axis it can be positioned on:
...
Using the `animation start time` and
`animation length` parameters you can create multiple animations. The unit for both parameters is seconds. Example:
```cpp
...
_orthogonal camera_, it's easy to do using
`Camera::createOrthographic()`. Example:
```cpp
auto s = Director::getInstance()->getWinSize();
