...
creating your games with Cocos2d-x. However, please realize there is no limit to what you can create. This chapter covers __advanced__ concepts. Note that this chapter gets more technical in its content and format.
...
The `FileUtils` class has been created to resolve these issues. `FileUtils` is a helper class to access files under
...
In Cocos2d-x, all `Node` objects that are renderables use shaders. As an example
