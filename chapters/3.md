...
#### What is Ploygon Sprite?
Polygon Sprite is also a sprite, use to display a 2d image.But unlike normal Sprite, which is a rectangle made of just 2 triangles, Polygon Sprite is made of a series of triangles.

#### Why you should use Ploygon Sprite? 
Performance!
...
####AutoPolygon
AutoPolygon is a help class, the purpose of it is to 
process an image into 2d polygon mesh in runtime.

It has functions for each step in the process, from tracing all the points, to triangulation, the result can be then passed to Sprite::create() to create a Polygon Sprite.
